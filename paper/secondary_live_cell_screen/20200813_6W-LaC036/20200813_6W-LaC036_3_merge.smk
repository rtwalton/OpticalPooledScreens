...
df_align = pd.read_hdf('fast_alignment.hdf').query(gate)
...
rule merge:
    ...
    run:
        df_ph_info = pd.read_csv(input[0])
        final = sorted(df_ph_info['frame'].unique())[-1]
        df_ph_info = df_ph_info.query('frame==@final')
        df_sbs_info = pd.read_csv(input[1]).rename(columns={'i':'j','j':'i'})
        Snake.merge_triangle_hash(output=output,df_0=df_ph_info,df_1=df_sbs_info, threshold=threshold,
