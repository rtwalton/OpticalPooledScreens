...
#     print(matches, df_info_0,df_info_0)
...
def multistep_alignment(df_0, df_1, df_info_0, df_info_1, 
    ...
    # d0 and d1 are unpacked from the det_range tuple
    d0, d1 = det_range

    # gate is a string that represents a condition for selecting rows from df_initial
    # It filters rows based on the determinant and score values
    gate = '@d0 <= determinant <= @d1 & score > 0.1'

    # alignments is a list containing a single DataFrame, which is a subset of df_initial
    # It includes only the rows that satisfy the condition specified in the 'gate'
    alignments = [df_initial.query(gate)]
    
    print("Alignments:", alignments)

    # Iteration begins, and this loop continues indefinitely until a break statement is encountered
    while True:
        df_align = (pd.concat(alignments, sort=True)
                    .drop_duplicates(['tile', 'site']))
        # print("df_aligned:", df_align)
        ...
        # Print keys in d_0 and d_1
        # print("Keys in d_0:", list(d_0.keys()))
        # print("Keys in d_1:", list(d_1.keys()))
