...
rule apply_illumination_correction:
    ...
    output:
        temp('process_ph/images/20X_{well}_Tile-{tile}.Channel-{channel}.corrected.tif')
    run:
        print(input[0])
        print(input[1])
...
rule correct_align:
    input:
        expand('process_ph/images/20X_{{well}}_Tile-{{tile}}.Channel-{channel}.corrected.tif',channel=CHANNELS)
    output:
        temp('process_ph/images/20X_{well}_Tile-{tile}.aligned.tif')
    run:
        stacked = Snake.stack_channels(data=input)
...
rule segment:
    input:
        'process_ph/images/20X_{well}_Tile-{tile}.aligned.tif'
    output:
