...
preprocess_pattern = 'input_sbs_tif/10X_c{cycle}-SBS-{cycle}_{{well}}_Tile-{{tile}}.sbs.Channel-{channel}.tif'
...
SEGMENTATION_CYCLE = 0
DAPI_INDEX = 0
CYTO_CHANNEL = 4
SEGMENT_METHOD = "cellpose"
NUCLEI_DIAMETER = 10.8
CELL_DIAMETER = 18.4
CYTO_MODEL = "cyto3"
...
rule align:
    priority: -1
    input:
        ([[preprocess_pattern.format(cycle=SBS_CYCLES[0], channel=CHANNELS[0])]]+
            [[preprocess_pattern.format(cycle=SBS_CYCLES[cycle], channel=channel) for channel in CHANNELS[cycle]] for cycle in range(1,len(SBS_CYCLES))])
    output:
